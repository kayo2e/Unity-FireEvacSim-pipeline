...
## 2026-09-14 — Stage 1 벽 검출을 직선(Hough Line) 기반으로 교체, WALL IoU 개선

### 한 일

**1. 기존 오탐(FP)의 원인을 원본 사진과 직접 대조.** Phase B의 FP 셀
170개 중 54개가 주변 반경 1칸 이내에도 정답 WALL이 전혀 없는 "고립 FP"
였다. 이 중 가장 큰 덩어리(약 30여 칸, 행 5-16·열 8-11)를 원본 사진에서
해당 픽셀 좌표로 잘라 확인하니 화장실 구역(남녀 화장실·휠체어 칸)과
정확히 일치했다. 칸막이 자체는 실제 벽이지만 정답은 이 구역을 통째로
하나의 열린 공간으로만 표시했고(40×25 해상도에서 칸 하나하나가 셀보다
작아 정답 제작 시 구조적 블록 단위로 근사함), 그 안의 사람·휠체어
픽토그램까지 크기 기준 필터를 통과해 벽으로 오탐되고 있었다. 즉 색상·
임계값 튜닝으로는 못 고치는 유형의 오차라는 걸 확인.

**2. Hough Line Transform 기반 벽 검출로 교체.** 라벨링된 대피안내도
데이터셋이 없어 CubiCasa5K류 학습 기반 세그멘테이션을 새로 훈련하거나
기존 사전학습 모델을 가져다 쓰는 건(도메인이 다른 장르라 그대로 적용 시
오히려 저하 위험) 이 시점에 비현실적이라 판단해, 라스터 도면 벡터화
문헌(Liu et al. 2017 등)이 쓰는 "벽은 직선, 아이콘·해칭은 곡선/복잡한
외곽선" 원칙을 적용했다. `cv2.HoughLinesP`로 어두운 픽셀 마스크에서
축 정렬(0°/90° ± 8°) 직선 성분만 벽 후보로 인정하고, 건물 외곽 판정은
기존의 촘촘한 마스크로 그대로 유지했다(선분만 남은 성긴 마스크로는
외곽 컨투어 검출이 깨지는 걸 확인해 두 역할을 분리함).

1차 시도(파라미터 튜닝 전)는 오히려 WALL IoU가 0.590까지 떨어졌다 —
엘리베이터·소화기 표지판처럼 직선 위주 아이콘(사각형 프레임)까지는
이 방법으로 안 걸러진다는 걸 뒤늦게 확인. `min_line_length`·
`max_line_gap`·`angle_tol_deg`·`line_thickness`를 그리드 서치(WALL IoU
기준)로 다시 찾은 결과(`min_line_length=14, max_line_gap=6,
angle_tol_deg=8, line_thickness=1`) 전 지표가 개선됐다.

| 지표 | 기존(크기 기반) | 신규(직선 기반) |
|---|---:|---:|
...
WALL recall만 소폭 하락(가는 1px 선이 실제 벽 일부를 놓침)했고, 특히
RL 환경 관점에서 직접적으로 의미 있는 연결성 보존율(+9.9%p)과 자체
출구 도달률(100%)이 개선됐다.

**3. 프로덕션 코드 반영.** `stage1/build_base_grid.py`의 `load_wall_mask()`
를 이 방식으로 교체하고, 기존 방식은 `load_wall_mask_legacy_blob()`으로
남겨 비교/회귀 테스트용으로 보존했다. `base_grid.npy`/`base_grid_vis.jpg`
재생성 후 `eval_extraction_accuracy.py`로 재검증. `docs/stage1-extraction-accuracy.md`
에 Phase E로 기록, README의 Stage 1 관련 두 문단 갱신.

**한계**: 도면이 축 정렬(orthogonal) 구조라는 전제가 깔려 있고,
하이퍼파라미터를 이 사진 한 장 기준으로 그리드 서치했다(문 기호 템플릿
매칭과 같은 종류의 과적합 위험). 엘리베이터·소화기 표지판 같은 직선
위주 아이콘은 여전히 구분이 안 된다 — 진짜로 의미 기반 구분이 필요하면
결국 학습된 표현(CubiCasa5K류)이 필요하다는 결론은 그대로 유효하다.

### Next

- 두 번째 건물 사진으로 이 방법(축 정렬 전제, 그리드 서치된 하이퍼파라미터)
  이 일반화되는지 확인. 사용자가 새 사진을 제공해야 진행 가능.
- Stage 1(`base_grid.npy`)과 Stage 2 학습 그리드(`env_core.BASE_GRID`) 실제
  연결은 여전히 보류 상태 — 이번 개선으로 그리드 품질 우려가 줄었으니
  재검토 여지가 생겼다.
