...
추출)은 같은 건물 사진 한 장을 별도로 검증했다(연결성 98.1%, 출구 검출
100%. 상세: [Stage 1 추출 정확도 평가](docs/stage1-extraction-accuracy.md)).
두 구성 요소를 하나로 잇는 건 다음 단계다.
...
관계없이 속도가 일정하다(O(1)). A\*와 Hazard-aware BFS는 둘 다 에이전트마다
개별 탐색을 돌려야 해서 O(N)으로 선형 증가한다. N=20/50/100/150/200/300/500
7단계로 재실측했다(`astar_real.py` Pure A\*, `astar_baseline.py` Hazard-aware
BFS, mean 액션결정시간, M1 8코어 CPU. Hazard-aware BFS는 생존율 비교에서
PPO와 가장 근접한 비학습 경쟁자라 이번에 추가 측정했다 — 상세:
[Hazard-Awareness Ablation](docs/hazard-aware-ablation.md) 후속 분석 7).

| N | A\* mean | Hazard-aware BFS mean | PPO mean |
| :---: | :---: | :---: | :---: |
| 20 | 4.2ms | 8.4ms | 0.23ms |
| 50 | 7.8ms | 19.3ms | 0.17ms |
| 100 | 20.1ms | 40.1ms | 0.14ms |
| 150 | 23.0ms | 59.1ms | 0.25ms |
| 200 | 28.8ms | 65.0ms | 0.14ms |
| 300 | 53.7ms | 125.8ms | 0.21ms |
| 500 | 114.1ms | 281.5ms | 0.51ms |

> PPO는 N과 무관하게 0.5ms 이하로 일정(O(1) 실측 확인). Hazard-aware BFS는
> 생존자마다 출구 A·B 양쪽으로 탐색을 새로 돌리는 구조라 Pure A\*보다도
> 대략 2배 느리다. **"실시간" 기준을 100ms/스텝으로 잡으면 Hazard-aware
> BFS는 N≈200~300 사이에서 이미 이 기준을 넘고, Pure A\*는 그보다 더 큰
> N에서 넘는다.** PPO는 이 기준을 넘길 일이 구조적으로 없다. 절대 시간은
> 실행 시점의 하드웨어 부하에 따라 달라질 수 있으나(위 표는 이전 표 3의
> 측정 회차와 다른 회차다), O(1) vs O(N)이라는 정성적 차이와 순서(PPO ≪
> A\* < Hazard-aware BFS)는 두 회차 모두 일관됐다.
...
**피처 공간(F1~F15) 최적화**: 관측 벡터 15차원은 초기 설계 이후 재검토 없이
쓰여왔다. permutation 기반 개별·그룹 마스킹과 15×15 상관행렬 분석을 마쳤고
(F3·F7·F8·F10·F11이 "출구 선호"라는 같은 축을 서로 다르게 인코딩하는
등 중복 클러스터 확인, F1/F2와 F14/F15는 예상과 달리 독립적), 그 과정에서
피처 중복보다 더 큰 문제가 드러났다 — 마스킹해도 생존율이 거의 안 변한
진짜 이유는 정책이 학습 후에도 액션 경계값에 고정돼 있던 버그였다(위
"PPO 아키텍처 미검증" 항목 참고). 그 버그를 고친 뒤에도 F1~F15 각각의
개별 기여도를 축소 피처셋 재학습으로 분리하는 작업은 아직 남아 있다.
Kim & Ha(2020)는 observation dropout·탐색 기반 최적화로 불필요한 관측
채널을 제거해도 정책 성능이 유지되거나 개선됨을 보였고, 최근 군중 대피
MARL 연구들은 전역 상태 대신 국소 관측(localized observation)으로 계산
부담을 줄이는 방향을 택하는 경우가 많다. 본 프로젝트의 "전역 요약 통계
15차원" 설계가 이 축소 여지를 실제로 얼마나 갖는지는 아직 열려 있다.
경과와 다음 단계는 [피처 공간 최적화
