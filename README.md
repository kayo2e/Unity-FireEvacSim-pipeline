...
| S5 EXIT B 위협 | 40명 | 85±9 | 74±6 | 106±15 | 103±15 |
| S6 중앙 경로 차단 | 40명 | 85±10 | **91±6** | 81±12 | **77±12** |

> 핵심: S2·S3·S6에서 PPO가 A\* 대비 **최대 +12%p 생존율 향상** / 완료 시간 단축.
> PPO는 F7/F8(출구 혼잡도)을 실시간 인식해 병목 상황에서 출구 분산 유도. 다만
> **S4(양방향 동시 위협)에서는 생존율이 사실상 동률**(66.8% vs 65.8%,
> paired t-test p=0.41 — 유의하지 않음)이고, **S5(EXIT B 위협)에서는 PPO가
> A\*보다 낮음**(74% vs 85%) — 좁은 단일 출구·양방향 동시 위협 상황에서는
> A\*의 결정론적 경로가 유리할 수 있다는 정도로만 해석하며, 원인 분석은 추가
> 조사가 필요하다. † S4의 **완료 시간 차이는 통계적으로 유의**하다
> (paired t-test p=0.0072, Wilcoxon p=0.0075, n=30) — 생존율은 못 앞서도
> 탈출을 유의하게 더 빨리 끝낸다는 뜻으로, "생존율은 동률, 속도는 유의미하게
> 빠름"이 S4에 대한 정확한 결론이다.
>
