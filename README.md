...
| S4 양방향 동시 위협 | 40명 | 60±37 | 60±34 | 77±16 | 72±16 |
...
> **S4(양방향 동시 위협)에서는 생존율이 사실상 동률**(59.7% vs 59.5%, 완료 시간만
> PPO가 소폭 빠름)이고, **S5(EXIT B 위협)에서는 PPO가 A\*보다 낮음**(74% vs 85%)
> — 두 시나리오 모두 편차가 매우 커서(σ 30%대 이상) 30 에피소드로는 결론을
> 단정하기 어렵고, 좁은 단일 출구·양방향 동시 위협 상황에서는 A\*의 결정론적
> 경로가 유리할 수 있다는 정도로만 해석한다. 원인 분석은 추가 조사가 필요하다.
