...
**PPO 아키텍처·커리큘럼 설계 미검증**: 지금 PPO는 기본 MLP(`net_arch=[256,
256]`) 구조 그대로이고, 지역성 요약 레이어 같은 경량 모듈 추가는 아직
실험하지 않았다. 커리큘럼 학습(S1→S4 순차 진급) 자체가 실제로 효과가
있는지도, 같은 조건에서 커리큘럼 없이 학습한 모델과 비교한 적이 없다.
엔트로피 계수(`ent_coef`)는 문헌 대조 감사를 마쳤지만(기존 기본값 0.05
유지가 더 나음을 확인, 상세는 [Hazard-Awareness
Ablation](docs/hazard-aware-ablation.md) 후속 분석 4), GAE λ·learning
rate schedule 등 다른 하이퍼파라미터는 아직 검토하지 않았다.
