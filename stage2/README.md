...
### 최신 결과 (1,000 에피소드, ppl12 모델)

> 조건: 10명, 각 시나리오 **1,000 에피소드** / PPO 모델: `fire_evac_model_10ppl` (ppl12, S4 난이도 완화 재학습)  
...
- **S1·S2**: 통계적으로 동률 — 단순 화재 환경에서는 규칙 기반과 학습 기반 모두 동등
- **S3 (출구 위협)**: A*가 12%p 우세 — 출구 전환 타이밍을 규칙으로 즉각 판단. PPO는 3600차원 픽셀에서 화재 위협 신호 추출에 어려움
- **S4 (폭발 붕괴)**: PPO가 **21.5%p** 우세 — 복잡한 다중 화재·붕괴 패턴에서 PPO의 학습된 전략이 고정 규칙 대비 크게 유리. A* 평균 스텝 592.2(거의 전 에피소드 타임아웃)인 반면 PPO는 459.4로 절반 가량 조기 탈출 성공
- **S3 개선 방향**: obs 3606차원(화재 위협 요약 피처 포함) + 출구 분산 보상으로 재학습 시 역전 기대
