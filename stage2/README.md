...
> 조건: 10명, 각 시나리오 30 에피소드 / PPO 모델: `fire_evac_model_10ppl` (구 obs 3600차원)  
> A* 베이스라인: 동일한 환경·유도등 제어 메커니즘, 화재 위협 기반 규칙으로 비용 결정

| 시나리오 | 설명 | PPO 생존율 | A* 생존율 | 차이 | 승자 |
| :---: | :--- | :---: | :---: | :---: | :---: |
| S1 | 초기 화재 | **100.0%** | **100.0%** | ±0 | 동률 |
| S2 | 화재 확산 | **95.7%** | 93.0% | +2.7%p | **PPO** |
| S3 | 출구 위협 | 76.7% | **90.0%** | −13.3%p | **A*** |
| S4 | 폭발 붕괴 | **40.3%** | 29.0% | +11.3%p | **PPO** |

**세부 지표 (생존율 mean ± std)**

| 시나리오 | PPO 생존율 | PPO 탈출 | PPO 스텝 | A* 생존율 | A* 탈출 | A* 스텝 |
| :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| S1 | 1.000 ± 0.000 | 10.0 | 17.2 | 1.000 ± 0.000 | 10.0 | 19.7 |
| S2 | 0.957 ± 0.072 |  9.6 | 20.8 | 0.930 ± 0.101 |  9.3 | 25.5 |
| S3 | 0.767 ± 0.114 |  7.7 | 20.1 | 0.900 ± 0.078 |  9.0 | 22.6 |
| S4 | 0.403 ± 0.226 |  4.0 | 600 (타임아웃) | 0.290 ± 0.190 |  2.9 | 600 (타임아웃) |

**분석**

- **S1·S2**: PPO가 동등하거나 우세 — 단순 화재 환경에서 빠른 경로 학습
- **S3 (출구 위협)**: A*가 13%p 우세 — 출구 전환 타이밍을 규칙으로 즉각 판단하는 A*가 유리. PPO는 학습 부족 또는 obs에서 화재 위협 신호 추출 어려움
- **S4 (폭발 붕괴)**: PPO가 11%p 우세 — 복잡한 다중 화재·붕괴 패턴에서 PPO의 학습된 전략이 고정 규칙 대비 유리
