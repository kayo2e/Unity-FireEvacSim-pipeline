...
> A* S1: `result/astar_baseline/test_results_s1_10ppl_20260505_141017.csv` (2026-05-05 신규 실행)  
> A* S2~S4, PPO 전 시나리오: 이전 실행 결과 (동일 병목 환경)
...
> † S4는 30 에피소드 표본이 작아 통계적 유의성 낮음 (std ≈ 0.29)
...
> ★ S1 A* 상세 (2026-05-05): 생존율 1.0000, 스텝 45.27 ± 12.51, 보상 458.81 ± 36.55, 최대 화재셀 6.63

**출구 분산 지표 (escaped_A : escaped_B) — 병목 환경 핵심 지표**

| 시나리오 | PPO A출구 | PPO B출구 | A* A출구 | A* B출구 | 비고 |
| :---: | :---: | :---: | :---: | :---: | :--- |
| S1 ★ | **4.9** | **5.1** | 3.87 | 6.13 | PPO가 거의 균등 분산 |
| S2 | **5.0** | 4.4 | 3.9 | 5.4 | PPO 균형 우위 |
| S3 | 0.5 | 7.9 | 0.1 | 9.6 | 양쪽 모두 B출구 집중 (A출구 화재 위협) |
| S4 | 1.9 | 2.6 | 2.1 | 2.9 | 생존율 자체가 낮아 분산 효과 제한적 |

> ★ S1 A* 분산 범위: escaped_A 1~8 (std 1.56), escaped_B 2~9 (std 1.56) — 30에피소드 기준

**분석**

- **S1 (초기 화재)**: 생존율 동률이나 PPO가 A:B = 4.9:5.1로 균등 분산, A*는 3.87:6.13으로 B출구 편중 — F7/F8 병목 피처의 효과 확인
- **S3 (출구 위협)**: A*가 **13.7%p** 우세 — 화재 위협 감지 즉시 규칙 기반으로 전 인원을 B출구로 전환하므로 대응 속도 빠름; PPO는 전환 타이밍 추가 학습 필요
- **S4 (폭발 붕괴)**: 30 에피소드 기준 A*가 소폭 우세하나 std ≈ 0.29로 통계적 의미 낮음
