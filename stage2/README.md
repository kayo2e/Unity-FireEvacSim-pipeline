...
| 2 | 점진적 위협 | 중앙 복도 무작위 | 10% | 250 | 10 | PPO 소폭 우세 (F14/F15 선제 대응) |
| 3 | 출구 혼잡 | 중앙 복도 무작위 | 5% | 180 | **20** | **PPO 명확히 우세** (F7/F8 혼잡 분산) |
| 4 | 부분 위협 | 출구 A·B 구역 각 60% 독립 점화 | 18% | 280 | **15** | **PPO 우세** (위협+혼잡 동시 trade-off) |

> **S3·S4 인원수**: 권장 인원수가 기본값. `--n` 미지정 시 자동 적용됨.  
...
> 시나리오 전면 개편 후 재측정 필요. PPO 재학습 완료 후 결과 업데이트 예정.  
> 공통 조건: 각 시나리오 **30 에피소드**, `EXIT_CAPACITY=1` / `CELL_CAPACITY=1` 병목 적용  
> 시나리오별 권장 인원수: S1·S2 → 10명, S3 → 20명, S4 → 15명

**예상 비교 방향**

| 시나리오 | 핵심 테스트 | 예상 우위 | 이유 |
| :---: | :--- | :---: | :--- |
| S1 기본 탈출 | 단순 경로 | 동률 | 화재 고정·단순, 둘 다 100% 예상 |
| S2 점진적 위협 | F14/F15 선제 반응 | PPO 소폭 | 위협 변화 속도 감지 후 선제 전환 가능 |
| S3 출구 혼잡 | F7/F8 혼잡 분산 | **PPO 명확** | A*는 F7/F8 미사용 → 병목 방치 |
| S4 부분 위협 | 위협+혼잡 trade-off | **PPO 우세** | 이진 결정이 아닌 복합 판단 필요 |

```bash
# A* 베이스라인 측정 (권장 인원수 자동 적용)
python astar_baseline.py --all-scenarios --episodes 30

# PPO 측정 (시나리오별 권장 인원수 지정)
python train.py --mode test --test-n 10  --test-scenario 1 --test-episodes 30
python train.py --mode test --test-n 10  --test-scenario 2 --test-episodes 30
python train.py --mode test --test-n 20  --test-scenario 3 --test-episodes 30
python train.py --mode test --test-n 15  --test-scenario 4 --test-episodes 30
```
